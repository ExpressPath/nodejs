...
} = require('./execution-delegate');

const JOB_TYPE_PROOF_CHECK = 'proof-check';
const JOB_TYPE_CONVERT = 'lambda-convert';
const JOB_TYPE_SUBMIT = 'submit';
...
async function performProofCheck(payload) {
  if (hasExecutionDelegate()) {
    return delegateProofCheck(payload || {});
  }
  return runProofCheck(payload || {});
}

async function runVerifiedConversion(payload) {
  const normalizedPayload = payload || {};
  if (hasExecutionDelegate()) {
    return delegateLambdaConversion(normalizedPayload);
  }
...
async function performLambdaConversion(payload) {
  return runVerifiedConversion(payload);
}
...
function getHelperInfo() {
  const delegated = hasExecutionDelegate();
  const proofRuntimes = getRuntimeInfo();
  const conversionRuntimes = getConverterInfo();

  if (delegated) {
    Object.values(proofRuntimes).forEach((runtime) => {
      if (runtime && typeof runtime === 'object') {
        runtime.upstream = 'delegated';
      }
    });

    if (conversionRuntimes.typedLambda) {
      Object.values(conversionRuntimes.typedLambda).forEach((runtime) => {
        if (runtime && typeof runtime === 'object') {
          runtime.upstream = 'delegated';
        }
      });
    ...
    if (conversionRuntimes.cic) {
      Object.values(conversionRuntimes.cic).forEach((runtime) => {
        if (runtime && typeof runtime === 'object') {
          runtime.upstream = 'delegated';
        }
      });
  ...
  return {
    executionDelegate: getExecutionDelegateInfo(),
    proofRuntimes,
    conversionRuntimes,
    executionModel: {
      proofCheck: delegated ? 'delegated' : 'local',
      conversion: delegated ? 'delegated' : 'local'
    }
  };
}
...
function extractResultMessage(result, fallbackMessage) {
  ...
  if (result.stage === 'conversion' && result.conversion) {
    ...
    if (conversion.lambda && typeof conversion.lambda.error === 'string' && conversion.lambda.error.trim()) {
...
async function executeSubmitJob(payload) {
  const result = await performSubmit(payload);
  if (!result.ok) {
    throw createResultError('Submit failed', result);
  }
  return result;
}
