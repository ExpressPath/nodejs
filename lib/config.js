...
const config = {
  ...
  serviceVersion: process.env.SERVICE_VERSION || '1.6.0',
  ...
  executionDelegate: {
    baseUrl: normalizeBaseUrl(
      ...
      || process.env.RENDER_EXECUTOR_BASE_URL
      || ''
    ),
    apiKey: process.env.EXECUTION_SERVER_API_KEY || process.env.RENDER_EXECUTOR_API_KEY || '',
    timeoutMs: parsePositiveInt(
      process.env.EXECUTION_SERVER_TIMEOUT_MS || process.env.RENDER_EXECUTOR_TIMEOUT_MS,
      180000
    ),
    enabled: false
  },
...
config.executionDelegate.enabled = Boolean(config.executionDelegate.baseUrl);
